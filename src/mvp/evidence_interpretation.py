...
retrieved evidence, and historical context, and may return only narrative
fields plus evidence IDs. The module has no model SDK dependency and always
falls back to calibrated deterministic text when credentials, a provider, or a
...
INTERPRETATION_PROMPT_VERSION = "evidence-interpretation-prompt-v1"
DETERMINISTIC_INTERPRETATION_VERSION = "deterministic-evidence-interpretation-v1"
INTERPRETATION_INSTRUCTIONS = """\
Return only the eight EvidenceInterpretation narrative fields. Use only the
supplied deterministic signals, retrieved evidence, and historical context.
Cite evidence by supplied evidence_id only. Do not calculate or restate
numbers, alter values or trigger states, add external facts or articles, assert
causality or crash certainty, or give portfolio or trade recommendations.
Use calibrated monitoring language and state uncertainty explicitly."""
    ...
    *,
    ...
    if triggered:
        narrative_state = (
            ...
            "least one implemented fragility condition is triggered. This is "
            "not a confirmed crash forecast."
        )
    else:
        narrative_state = (
            "No implemented fragility condition is currently triggered. This "
            "does not rule out risks outside the monitored signal set."
        )
    ...
    return EvidenceInterpretation(
        ...
        narrative_changes=_deterministic_changes(deterministic_input),
        supporting_evidence_ids=supporting_ids,
        contradicting_evidence_ids=contradicting_ids,
        missing_or_uncertain_evidence=_unique(missing),
        pm_interpretation=evidence_view,
        monitoring_questions=(
...
def _validated_provider_result(
    ...
    if not 3 <= len(candidate.monitoring_questions) <= 5:
        raise ValueError("LLM output must contain 3 to 5 monitoring questions")
    if not 2 <= len(candidate.invalidation_conditions) <= 4:
        raise ValueError("LLM output must contain 2 to 4 invalidation conditions")
...
def interpret_evidence_card(
    ...
    *,
    ...
    if not use_llm:
        return _deterministic_interpretation(deterministic_input)
    ...
    try:
        raw = interpreter.interpret(
            context=_model_context(deterministic_input),
